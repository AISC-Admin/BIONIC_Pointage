...
export async function GET() {
  ...
  const { rows } = await sql`
    SELECT id, nom, prenom, to_char(date_naissance, 'YYYY-MM-DD') AS date_naissance,
      telephone, email, ville, poste, langues, taux_horaire, dispo_ete, dispo_hiver,
...
export async function POST(request) {
  ...
  const { rows } = await sql`
    INSERT INTO base_salaries (nom, prenom, date_naissance, telephone, email, ville, poste, langues,
      taux_horaire, dispo_ete, dispo_hiver, cv_texte, cv_url, cv_nom, photo_url)
    VALUES (${f.nom}, ${f.prenom}, ${f.date_naissance}, ${f.telephone}, ${f.email}, ${f.ville}, ${f.poste},
      ${f.langues}, ${f.taux_horaire}, ${f.dispo_ete}, ${f.dispo_hiver}, ${f.cv_texte}, ${f.cv_url},
      ${f.cv_nom}, ${f.photo_url})
    RETURNING id;
  `;
