...
// elle n'a pas deja ete validee (ou corrigee) par le responsable.
export async function DELETE(request, { params }) {
  ...
  const { rows } = await sql`
    SELECT id, valide, modifie_par_manager FROM shifts WHERE id = ${id} AND employee_id = ${session.employeeId} LIMIT 1;
  `;
  if (rows.length === 0) {
    return NextResponse.json({ erreur: 'Vacation introuvable.', code: 'shift_not_found' }, { status: 404 });
  }
  if (rows[0].valide || rows[0].modifie_par_manager) {
